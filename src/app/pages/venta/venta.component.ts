...
export class VentaComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this._configService.leerParametro(environment.PRECIO_ENTRADA).subscribe(data => {
      this.precioEntrada = +data.valor; //el mas antepuesto a un string castea a numero
    });
  ...
  listarClientes() {
    this._clienteService.listar().subscribe(data => {
      this.clientes = data;
    });
  }
  ...
  listarComidas() {
    this._comidaService.listar().subscribe(data => {
      ...
      for (let c of this.comidas) {
        this._comidaService.listarPorId(c.idComida).subscribe(data => {
          //aqui tengo las imagenes
          let reader = new FileReader();
          reader.readAsDataURL(data);
          reader.onloadend = () => {
            let x = reader.result;
            //aqui ya tengo la imagen
  ...
  //lo sanitizo para mostrar la imagen
  setear(x: any) {
  ...
  registrar() {
    ...
    this._ventaService.registrar(ventaDTO).subscribe(data => {
      if (data === 1) {
  ...
  //aqui creo una a href y lo oculto para que
  generarReporte(ventaDTO: VentaDTO) {
    this._ventaService.generarReporte(ventaDTO).subscribe(data => {
